...
function TeacherDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      data.forEach(t => t.teacher?.forEach(tt => teachersSet.add(tt.name)));
    ...
    <div style={styles.wrapper}>
        ...
        <div style={styles.sidebar}>
          <div style={styles.profileWrapper}>
            <label className="photoBox">
              <img
                src={profileImg || "https://cdn-icons-png.flaticon.com/512/3135/3135715.png"}
          ...
          <button style={styles.homeBtn} onClick={goHome}><FaHome /> Home</button>
          <button style={styles.logoutBtn} onClick={logout}><FaSignOutAlt /> Logout</button>
        </div>
      )}
      ...
      <div style={styles.main}>
        ...
        <div style={styles.card}>
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {times.map((timeKey, i) => {
                // ✅ FIXED: split "||" for match + display
                const [start, end] = timeKey.split("||");
                return (
                  <tr key={i}>
                    {/* ✅ FIXED: 12hr format display */}
                    <td>{formatTime(start)} – {formatTime(end)}</td>

                    {(selectedDay ? [selectedDay] : days).map(day => {
                      const match = timetable.find(
                        t =>
                          t.timeslot?.startTime === start &&
                          t.timeslot?.endTime === end &&
                          t.timeslot?.day === day
                      );
      ...
      <style>{`
        .photoBox {
          ...
          position: relative; cursor: pointer; display: block; margin: auto;
        }
        .profileImg { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; }
        .cameraIcon {
        ...
        .photoBox:hover .cameraIcon { opacity: 1; }
      `}</style>
...
const styles = {
  ...
  input:{ width:"100%", padding:"10px 40px 10px 10px" },
  icon:{ position:"absolute", right:"12px", top:"50%", transform:"translateY(-50%)", color:"#666" },
