...
// ✅ ADDED: "09:00||10:00" → "9:00 AM – 10:00 AM"
...
function TimetableGrid() {
  ...
  // ✅ FIXED: key is "startTime||endTime" (same as TimetableView)
  const times = [
    ...new Set(
      data
        .filter(item => item.timeslot?.startTime && item.timeslot?.endTime)
        .map(item => item.timeslot.startTime + "||" + item.timeslot.endTime)
    )
  ].filter(Boolean).sort((a, b) => a.split("||")[0].localeCompare(b.split("||")[0]));
  ...
  // ✅ FIXED: match using || key
  const getCellData = (day, timeKey) => {
    ...
    <div className="tt-container">
      ...
      <div className="tt-table">
        ...
        <div className="tt-row header">
          ...
          {/* ✅ FIXED: display formatted time, key is || separated */}
          {times.map(t => (
            <div key={t} className="tt-cell head">{formatTimeRange(t)}</div>
          ))}
        </div>
