...
function StudentDashboard() {
  ...
  // ✅ FIXED: "||" separator — same pattern as TimetableGrid & TimetableView
  const times = [...new Set(
    timetable
      .filter(t => t.timeslot?.startTime && t.timeslot?.endTime)
      .map(t => `${t.timeslot.startTime}||${t.timeslot.endTime}`)
  )].sort((a, b) => a.split("||")[0].localeCompare(b.split("||")[0]));

  // ✅ FIXED: grid key bhi "||" separator se
  const gridData = {};
  timetable.forEach(item => {
    const day = item.timeslot?.day;
    const timeKey = `${item.timeslot?.startTime}||${item.timeslot?.endTime}`;
    if (!gridData[day]) gridData[day] = {};
    gridData[day][timeKey] = item;
  });
    ...
    <div style={container}>
      ...
      <div style={card}>
        ...
        <div style={grid3}>
          ...
          <div>
            <input placeholder="Search Department..." value={deptSearch} onChange={(e)=>setDeptSearch(e.target.value)} style={input}/>
          ...
          <div>
            <input placeholder="Search Course..." value={courseSearch} onChange={(e)=>setCourseSearch(e.target.value)} style={input}/>
          ...
          <div>
            <input placeholder="Search Section..." value={sectionSearch} onChange={(e)=>setSectionSearch(e.target.value)} style={input}/>
        ...
        <div style={{ marginTop:"15px", display:"flex", gap:"10px" }}>
          <button onClick={fetchTimetable} style={btn}>🚀 View Timetable</button>
          ...
          <table style={table}>
            <thead>
              <tr style={{ background:"#1e3a8a", color:"white" }}>
                ...
                {/* ✅ FIXED: display 12hr format */}
                {times.map((t, i) => {
                  const [start, end] = t.split("||");
                  return <th key={i} style={th}>{formatTime(start)} – {formatTime(end)}</th>;
                })}
              </tr>
            </thead>
            ...
            <tbody>
              {days.map(day => (
                <tr key={day}>
                  ...
                  {times.map(timeKey => {
                    const item = gridData[day]?.[timeKey];
                    return (
                      <td key={timeKey} style={{ ...td, background: getColor(item?.subject?.type) }}>
                        {item ? (
                          <>
...
const header = { display:"flex", justifyContent:"space-between", alignItems:"center", background:"linear-gradient(45deg,#4f46e5,#6366f1)", color:"white", padding:"20px", borderRadius:"12px" };
const msg = { background:"#dcfce7", padding:"10px", marginTop:"10px", borderRadius:"8px" };
const card = { background:"white", padding:"20px", borderRadius:"12px", marginTop:"20px" };
const grid3 = { display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"10px" };
const input = { padding:"10px", borderRadius:"8px" };
const btn = { flex:1, background:"#6366f1", color:"white", padding:"10px", border:"none", borderRadius:"8px" };
const printBtn = { flex:1, background:"#16a34a", color:"white", padding:"10px", border:"none", borderRadius:"8px" };
const homeBtn = { background:"#22c55e", color:"white", padding:"8px 12px", border:"none", borderRadius:"8px" };
const logoutBtn = { background:"#ef4444", color:"white", padding:"8px 12px", border:"none", borderRadius:"8px" };
